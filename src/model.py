...
class DataTransformer:
    ...
    def __init__(
        self,
        job_id: str = None,
        train_data_shards_dir: str = None,
        categorical_list=[],
        mixed_dict={},
        n_clusters=10,
        eps=0.005,
        ...
        self.meta = None
        self.train_data_shard_paths = (
            glob(f"{train_data_shards_dir}/*.parquet")
            if train_data_shards_dir
            else train_data_shards_dir
        )
        self.job_id = job_id
        self.trained_model_artifacts = f"checkpoints/{job_id}"

        if job_id is None:
            raise "job_id cannot be None"
        self.create_model_artifact_dir()
    ...
    def create_model_artifact_dir(self):
        os.makedirs(self.trained_model_artifacts, exist_ok=True)

    def aggregate_metadata(self, shard_metadata_list):
        """
        Aggregates metadata from multiple shards.

        Parameters:
        - shard_metadata_list (list): List of metadata dictionaries for individual shards.

        Returns:
        - aggregated_meta (list): Aggregated metadata combining all shards.
        """
        aggregated_meta = {}
        for shard_meta in shard_metadata_list:
            for column_meta in shard_meta:
                column_name = column_meta["name"]
                if column_name not in aggregated_meta:
                    aggregated_meta[column_name] = column_meta
                else:
                    if column_meta["type"] == "categorical":
                        # Combine unique categories
                        aggregated_meta[column_name]["i2s"] = list(
                            set(
                                aggregated_meta[column_name]["i2s"] + column_meta["i2s"]
                            )
                        )
                        aggregated_meta[column_name]["size"] = len(
                            aggregated_meta[column_name]["i2s"]
                        )
                    elif (
                        column_meta["type"] == "mixed"
                        or column_meta["type"] == "continuous"
                    ):
                        # Update min/max values
                        aggregated_meta[column_name]["min"] = min(
                            aggregated_meta[column_name]["min"], column_meta["min"]
                        )
                        aggregated_meta[column_name]["max"] = max(
                            aggregated_meta[column_name]["max"], column_meta["max"]
                        )
        # Convert dict back to a list
        return list(aggregated_meta.values())

    def get_metadata(self):
        def process_single_shard(data):
            """Process metadata for a single shard."""
            meta = []
        ...
        if self.train_data_shard_paths:
            all_metadata = [
                process_single_shard(pd.read_parquet(train_data_shard_path))
                for train_data_shard_path in self.train_data_shard_paths
            ]

            # Aggregate metadata across all shards
            aggregated_meta = self.aggregate_metadata(all_metadata)
            self.save_metadata(aggregated_meta, self.trained_model_artifacts)
            return aggregated_meta
        else:
            return self.load_metadata(self.trained_model_artifacts)

    def save_metadata(self, aggregated_meta, file_path):
        with open(f"{file_path}/metadata.json", "w") as f:
            json.dump(aggregated_meta, f, indent=4)

    def load_metadata(self, file_path):
        with open(f"{file_path}/metadata.json", "r") as f:
            loaded_meta = json.load(f)
        return loaded_meta

    def fit(self):
        gm_continuous = BayesianGaussianMixture(
        ...
        gm1_mixed = BayesianGaussianMixture(
            n_components=self.n_clusters,
            weight_concentration_prior_type="dirichlet_process",
        ...
        gm2_mixed = BayesianGaussianMixture(
            n_components=self.n_clusters,
            weight_concentration_prior_type="dirichlet_process",
            weight_concentration_prior=0.001,
            max_iter=100,
        ...
        for train_data_shard_path in tqdm(
            self.train_data_shard_paths, total=len(self.train_data_shard_paths)
        ):
            data = pd.read_parquet(train_data_shard_path)
            ...
            for id_, info in enumerate(self.meta):
                if info["type"] == "continuous":
                    ...
                    gm_continuous.fit(data[:, id_].reshape([-1, 1]))
                    model.append(gm_continuous)

                    # keeping only relevant modes that have higher weight than eps and are used to fit the data
                    old_comp = gm_continuous.weights_ > self.eps
                    mode_freq = (
                        pd.Series(gm_continuous.predict(data[:, id_].reshape([-1, 1])))
                        .value_counts()
                        .keys()
                    )
                    comp = []
                    ...
                    # old_comp = gm_continuous.weights_ > self.eps
                    # mode_freq = np.bincount(gm_continuous.predict(data[:, id_].reshape([-1, 1])))
                    # comp = (old_comp & np.isin(range(self.n_clusters), mode_freq.nonzero()[0])).tolist()
                    ...
                    gm1_mixed.fit(data[:, id_].reshape([-1, 1]))
                    ...
                    gm2_mixed.fit(data[:, id_][filter_arr].reshape([-1, 1]))

                    model.append((gm1_mixed, gm2_mixed))

                    # similarly keeping only relevant modes with higher weight than eps and are used to fit strictly continuous data
                    old_comp = gm2_mixed.weights_ > self.eps
                    mode_freq = (
                        pd.Series(
                            gm2_mixed.predict(data[:, id_][filter_arr].reshape([-1, 1]))
                        )
                        .value_counts()
                        .keys()
                    )
        ...
        self.save_model(model, self.trained_model_artifacts)
        print(f"Model saved as {self.trained_model_artifacts}/fitted_model.pkl")

    def save_model(self, model, file_path):
        joblib.dump(model, f"{file_path}/fitted_model.pkl")

    def load_model(self, file_path):
        return joblib.load(f"{file_path}/fitted_model.pkl")

    def transform(self, data):
        model = self.load_model(self.trained_model_artifacts)
        ...
        for id_, info in enumerate(self.meta):
            ...
            if info["type"] == "continuous":
                ...
                means = model[id_].means_.reshape((1, self.n_clusters))
                stds = np.sqrt(model[id_].covariances_).reshape((1, self.n_clusters))
                ...
                means_0 = model[id_][0].means_.reshape([-1])
                stds_0 = np.sqrt(model[id_][0].covariances_).reshape([-1])
                ...
                stds = np.sqrt(model[id_][1].covariances_).reshape((1, self.n_clusters))

                features = np.empty(shape=(len(current), self.n_clusters))
                features = (current - means) / (4 * stds)

                n_opts = sum(self.components[id_])
                probs = model[id_][1].predict_proba(current.reshape([-1, 1]))
                probs = probs[:, self.components[id_]]
        ...
        output_file_path = f"output/transform_{self.job_id}"
        self.save_parquet_file(output_file_path, result_df)

        return output_file_path

    def inverse_transform(self, data):
        if not os.path.isdir(self.trained_model_artifacts):
            raise FileNotFoundError(
                f"The directory {self.trained_model_artifacts} does not exist."
            )
        ...
        for id_, info in enumerate(self.meta):
            if info["type"] == "continuous":
                ...
                means = model[id_][1].means_.reshape([-1])
                stds = np.sqrt(model[id_][1].covariances_).reshape([-1])
        ...
        result_df = pd.DataFrame(data_t)

        output_file_path = f"output/inverse_transform_{self.job_id}"
        self.save_parquet_file(output_file_path, result_df)

        return output_file_path

    def save_parquet_file(self, file_name, df):
